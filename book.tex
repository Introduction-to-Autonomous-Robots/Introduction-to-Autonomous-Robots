...
\chapter*{Preface}
This book provides an algorithmic perspective to autonomous robotics to students with a sophomore-level of linear algebra and probability theory. Robotics is an emerging field at the intersection of mechanical and electrical engineering with computer science. With computers becoming more powerful, making robots smart becomes more and more the focus of attention and one of robotics research. While there are a large number of textbooks on the mechanics and dynamics of robots that address sophomore-level undergraduates available, books that provide a broad algorithmic perspective are mostly limited to the graduate level. 
...
Also, the book is explicitely robot-agnostic, reflecting the timelessness of fundamental concepts. Instead, a series of possible project-based curricula are available online, ranging from a maze-solving competition that can be realized with most miniature differential-wheel robots that include a camera to manipulation experiments with the Baxter robot, all of which can be entirely conducted in simulation. 

Writing this book would not have been possible without the feedback of multiple cohorts of students first using the online lecture notes and later draft versions of this book. 
