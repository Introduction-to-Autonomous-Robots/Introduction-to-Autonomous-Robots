...
现在考虑以下初始信念状态分布：$p（1-2）=0.8$和$p（2-3）=0.2$。这里，$1-2$etc\指的是图\ref{fig:dervish_example}中的拓扑图上的位置。你知道机器人确实会面向东方。机器人现在开车一段时间，直到它报告“左侧敞开的走廊，右边的敞开的门”。这实际上对应于位置2，但机器人实际上可以在任何地方。例如，打开的门实际上是一个开放的走廊，即机器人真的位于4处有10％的机会。我们如何计算机器人位置的新概率分布？以下是可能发生的可能轨迹：
